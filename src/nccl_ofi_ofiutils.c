				    ...
				    unsigned int *num_prov_infos)
{
	...
	int max_dev_per_node = MAX_NUM_DEVICES_PER_NODE;
	...
	*num_prov_infos = 0;
	while (prov) {
		...
		} else if (*num_prov_infos >= max_dev_per_node) {
			fi_freeinfo(prov);
			NCCL_OFI_INFO(NCCL_INIT | NCCL_NET, "[INSTRUMENT] Skipping provider %s because we have exceeded the maximum number of devices per node specified in MAX_NUM_DEVICES_PER_NODE envvar or default (%d)",
				      prov->fabric_attr->prov_name, max_dev_per_node);
		}
		else {
